...
def start(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_markdown_v2(
        rf"Hola {user.mention_markdown_v2()}, me puedes preguntar por el nivel de polen y te responderé\. Los datos son de polenes\.cl, puedes visitarlos para tener más información\.",
...
def main() -> None:
    ...
    dispatcher.add_handler(MessageHandler(Filters.text & ~Filters.command, polenes))

    # Start the Bot
    updater.start_webhook(
        listen="0.0.0.0", port=PORT, url_path=TOKEN, webhook_url=URL + TOKEN
    )
